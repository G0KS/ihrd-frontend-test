import { createRef, useRef, useState } from "react";
import Details from "./Details";

function MovieGrid({ results }) {
   // state for details modal toggler
   const [isOpen, setIsOpen] = useState(false);
   // state for details modal data
   const [movie, setMovie] = useState({});

   // function to set values for the states movie and isOpen respectively
   const showDetails = (index) => {
      setMovie(results[index]);
      setIsOpen(true);
   };

   return (
      <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-3">
         {/* mapping through the results props to obtain the data */}
         {results.length > 0 ? (
            results.map(
               (
                  { id, poster_path, original_title, release_date, overview },
                  index
               ) => (
                  <div
                     className="bg-gray-600 p-2 rounded-lg movie-item"
                     key={id}
                  >
                     <img
                        src={"https://image.tmdb.org/t/p/w500" + poster_path}
                        alt={original_title}
                     />
                     <div className="font-bold text-white mt-2">
                        {original_title}
                     </div>
                     <div className="text-gray-400  ">{release_date}</div>
                     <div className="text-gray-400  h-6 overflow-ellipsis overflow-clip">
                        {overview}
                     </div>
                     {/* button to toggle the details modal */}
                     <button
                        className="bg-green-400 p-2 rounded-lg text-center mt-2 w-full font-bold"
                        onClick={() => showDetails(index)}
                     >
                        Details
                     </button>
                  </div>
               )
            )
         ) : (
            <>
               <div>No result found</div>
            </>
         )}
         {/* Details component called with setIsOpen and movie as props value for setOpenModal and movie respectively */}
         {isOpen && <Details setOpenModal={setIsOpen} movie={movie} />}
      </div>
   );
}

export default MovieGrid;
